...
var arc_mdpt, base, centers, fib, fib_cache, gr, group, initializePath, marker, mousePos, oc, offset, onMouseMove, onResize, scale, scale_text, settings, texts, transform, update_text, update_zoom, zoom;
...
scale_text = function(ratio) {
  ...
  for (_i = 0, _len = texts.length; _i < _len; _i++) {
    ...
    if (settings.numbers) {
      ...
      t.visible = t.data.n <= 1 ? zoom > 15 : t.data.marker.position.getDistance(marker.position) > 5;
      if (!t.visible) {
...
(function() {
  ...
  numbers = null;
  return gui.add(settings, 'infiniteIn').onChange(function(value) {
    update_zoom();
    if (value) {
      ...
      numbers.onChange(function(value) {
        update_text();
        scale_text(1);
      });
      console.log(numbers);
